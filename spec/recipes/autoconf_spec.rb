...
describe "chef-ruby::autoconf" do
  ...
  context "when autoconf is not already installed" do
    before :each do
      Chef::Resource::RemoteFile.any_instance.
          stub(:autoconf_already_installed?).with('2.69').and_return(false)
    ...
    it "downloads the latest autoconf source code version >=2.60" do
      expect(chef_run).to(
          create_remote_file("autoconf-2.69.tar.gz").with(
    ...
    it "installs autoconf" do
      expect(chef_run).to(
          run_execute("build-autoconf").with(
  ...
  context "when autoconf is installed" do
    before :each do
      Chef::Resource::RemoteFile.any_instance.
          stub(:autoconf_already_installed?).with('2.69').and_return(true)
      Chef::Resource::Execute.any_instance.
          stub(:autoconf_already_installed?).with('2.69').and_return(true)
    end
